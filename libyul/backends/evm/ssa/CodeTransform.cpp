...
DEFINE_LOGGER(codeTransformLogger, "yul.ssa.codetransform");
DEFINE_LOGGER(codeTransformDotLogger, "yul.ssa.codetransform.dot");
DEFINE_LOGGER(codeTransformShufflerLogger, "yul.ssa.codetransform.shuffler");
...
void AssemblyCallbacks::dup(StackDepth const _depth)
{
...
{
	solDebug(codeTransformLogger, "\n\n\n--------------------\nRunning SSA CFG code transform\n--------------------");
	solDebug(codeTransformDotLogger, "{}", _controlFlowLiveness.toDot());
...
{
	solDebug(codeTransformLogger, "Code transform for {}", m_cfg.function ? m_cfg.function->name.str() : "main");
...
void CodeTransform::operator()(SSACFG::BlockId const _blockId)
{
	...
	solDebug(codeTransformLogger, "\tGenerating for Block {} with label {}", _blockId.value, m_blockLabels[_blockId.value]);
	...
	for (std::size_t operationIndex = 0; operationIndex < block.operations.size(); ++operationIndex)
	{
		...
		solDebug(codeTransformLogger, "\t\t{}: {} -> {}", operationName(m_cfg.operation(block.operations[operationIndex])), stackToString(m_stack.data()), stackToString(operationInLayout));
	...
	auto const exitShuffleResult = traceShuffle(
		codeTransformShufflerLogger,
		3,
		m_stack,
		blockLayout->exitIn
	);
	yulAssert(exitShuffleResult.status == StackShufflerResult::Status::Admissible);
...
void CodeTransform::operator()(SSACFG::OperationId _opId, StackData const& _operationInputLayout)
{
	...
	auto const opInputShuffleResult = traceShuffle(
		codeTransformShufflerLogger,
		3,
		m_stack,
		_operationInputLayout
	);
	yulAssert(opInputShuffleResult.status == StackShufflerResult::Status::Admissible);
	...
	std::visit(util::GenericVisitor{
		[&](SSACFG::BuiltinCall const& _builtin) {
			solDebugRaw(codeTransformLogger, "\t\t\tBuiltin call: {}: {}", _builtin.builtin.get().name, stackToString(m_stack.data()));
		...
		[&](SSACFG::Call const& _call) {
			...
			if (returnLabel)
				solDebugRaw(codeTransformLogger, "\t\t\tCall: {} (label={}): {}, returnLabel: {}",
					_call.function.get().name.str(),
					m_functionLabels.at(&_call.function.get()),
					stackToString(m_stack.data()),
					*returnLabel);
			else
				solDebugRaw(codeTransformLogger, "\t\t\tCall: {} (label={}): {}",
					_call.function.get().name.str(),
					m_functionLabels.at(&_call.function.get()),
					stackToString(m_stack.data()));
			m_assembly.setSourceLocation(opOriginLocation);
		...
		[&](SSACFG::LiteralAssignment const&) {
			solDebugRaw(codeTransformLogger, "\t\t\tLiteral assignment: {}", stackToString(m_stack.data()));
		}
	}, _operation.kind);
	...
	solDebug(codeTransformLogger, " -> {}", stackToString(m_stack.data()));
...
void CodeTransform::operator()(SSACFG::BlockId const& _currentBlock, SSACFG::BasicBlock::ConditionalJump const& _conditionalJump)
{
...

	{
		...
		solDebug(codeTransformLogger, "\t\tJUMPI creating stack for zero layout (to Block {}) {} -> {}",
...
{
	yulAssert(static_cast<int>(m_stack.size()) == m_assembly.stackHeight());
	solDebug(codeTransformLogger, "\t\tJUMP creating target stack for jump {} -> {}", _currentBlock.value, _jump.target.value);
...
void CodeTransform::prepareBlockExitStack(StackData const& _target, PhiInverse const& _phiInverse)
{
	...
	// shuffle to target
	auto const shuffleResult = traceShuffle(
		codeTransformShufflerLogger,
		3,
		m_stack,
		pulledBackTarget
