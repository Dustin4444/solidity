...
struct InstructionStackInBuilder
{
	...
	/// - it is a non-literal value with a copy on the stack (literals are re-generated, never moved),
	...
	void dropForReach(StackData& _data, StackOffset const& _offset, OutOfReachFunction const& _outOfReach) const
	{
	...
	StackData build() const
	{
		...
		// simulate tail elements being swapped up into args
		for (std::size_t i = 0; i < args.size(); ++i)
		{
			// a moved copy on top leaves directly rather than being swapped down into a hole
			while (takeTop()) {}
			if (!pending[i])
				continue;
