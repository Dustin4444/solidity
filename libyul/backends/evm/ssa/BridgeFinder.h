	...
	)
	{
		...
		if (!m_visited[_neighbor.value])
		{
			...
			if (m_low[_neighbor.value] > m_disc[_vertex.value])
			{
				...
				bool const edgeNeighborToVertex = std::find(_vertexEntries.begin(), _vertexEntries.end(), _neighbor) != _vertexEntries.end();
				bool const edgeVertexToNeighbor = std::find(m_cfg.block(_neighbor).entries.begin(), m_cfg.block(_neighbor).entries.end(), _vertex) != m_cfg.block(_neighbor).entries.end();
