...
YulStack::assembleWithDeployed(std::optional<std::string_view> _deployName)
{
	...
	try
	{
		...
		if (debugInfoSelection().ethdebug)
			creationObject.ethdebug = evmasm::ethdebug::program(creationObject.assembly->name(), 0, *creationObject.assembly, *creationObject.bytecode.get());
		...
		if (deployedAssembly)
		{
			...
			if (debugInfoSelection().ethdebug)
				deployedObject.ethdebug = evmasm::ethdebug::program(deployedObject.assembly->name(), 0, *deployedObject.assembly, *deployedObject.bytecode.get());
