/*
	This file is part of solidity.

	solidity is free software: you can redistribute it and/or modify
	it under the terms of the GNU General Public License as published by
	the Free Software Foundation, either version 3 of the License, or
	(at your option) any later version.

	solidity is distributed in the hope that it will be useful,
	but WITHOUT ANY WARRANTY; without even the implied warranty of
	MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
	GNU General Public License for more details.

	You should have received a copy of the GNU General Public License
	along with solidity.  If not, see <http://www.gnu.org/licenses/>.
*/
// SPDX-License-Identifier: GPL-3.0

#pragma once

#include <libsolutil/Common.h>
#include <libsolutil/JSON.h>

#include <concepts>
#include <optional>
#include <variant>
#include <vector>

namespace solidity::evmasm::ethdebug::schema
{

struct EthdebugException: virtual util::Exception {};

struct HexValue
{
	bytes value;
};

struct NonNegativeValue
{
	template<std::unsigned_integral T>
	NonNegativeValue(T const _value)
	{
		solRequire(static_cast<T>(_value) <= std::numeric_limits<std::uint64_t>::max(), EthdebugException, "Too large value.");
		value = static_cast<std::uint64_t>(_value);
	}
	template<std::signed_integral T>
	NonNegativeValue(T const _value)
	{
		solRequire(_value >= 0, EthdebugException, "NonNegativeValue got negative value.");
		solRequire(static_cast<std::make_unsigned_t<T>>(_value) <= std::numeric_limits<std::uint64_t>::max(), EthdebugException, "Too large value.");
		value = static_cast<std::uint64_t>(_value);
	}
	NonNegativeValue(HexValue&& _value): value(std::move(_value)) {}

	std::variant<std::uint64_t, HexValue> value;
};

struct ID
{
	std::variant<std::string, std::uint64_t> value;
};

struct Source
{
	ID id;
};

struct Range
{
	NonNegativeValue length;
	NonNegativeValue offset;
};

struct SourceRange
{
	Source source;
	std::optional<Range> range;
};

struct ContextVariable
{
	std::optional<std::string> identifier;
	std::optional<SourceRange> declaration;
	// todo type
	// todo pointer according to ethdebug/format/spec/pointer
};

struct Context
{
	std::optional<SourceRange> code;
	std::optional<std::vector<ContextVariable>> variables;
	std::optional<std::string> remark;
};

struct Operation
{
	std::string mnemonic;
	std::vector<NonNegativeValue> arguments;
};

struct Instruction
{
	NonNegativeValue offset;
	std::optional<Operation> operation;
	Context context;
};

struct Contract
{
	std::optional<std::string> name;
	SourceRange definition;
};

enum class Environment
{
	CALL, CREATE
};

struct Program
{
	std::optional<Source> id;
	Contract contract;
	Environment environment;
	std::optional<Context> context;
	std::vector<Instruction> instructions;
};

/// serialization

void to_json(Json& _json, HexValue const& _hexValue);
void to_json(Json& _json, NonNegativeValue const& _nonNegativeValue);
void to_json(Json& _json, ID const& _id);
void to_json(Json& _json, Source const& _source);
void to_json(Json& _json, Range const& _range);
void to_json(Json& _json, ContextVariable const& _contextVariable);
void to_json(Json& _json, SourceRange const& _sourceRange);
void to_json(Json& _json, Context const& _context);
void to_json(Json& _json, Operation const& _operation);
void to_json(Json& _json, Instruction const& _instruction);
void to_json(Json& _json, Contract const& _contract);
void to_json(Json& _json, Environment const& _environment);
void to_json(Json& _json, Program const& _program);

}
